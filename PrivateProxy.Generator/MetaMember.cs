...
public class MetaMember
{
    ...
    public ImmutableArray<ITypeParameterSymbol> GenericParameters { get; } = ImmutableArray<ITypeParameterSymbol>.Empty;
    ...
    public MetaMember(ISymbol symbol)
    {
        ...
        else if (symbol is IMethodSymbol m)
        {
            ...
            GenericParameters = m.TypeParameters;
            ConstraintsString = EmitHelper.GetConstraintsString(GenericParameters);
        }
        else
        {
    ...
    public MetaMember(IMethodSymbol symbol, ImmutableArray<ITypeParameterSymbol> genericParams,string constraints)
    {
        this.Name = symbol.Name;
        this.IsStatic = symbol.IsStatic;

        {
            ...
            this.MemberType = symbol.ReturnType;
            this.MethodParameters = symbol.Parameters;
            this.MemberKind = MemberKind.Method;

            (this.IsRefReturn, this.IsRequireReadOnly) = (symbol.ReturnsByRef, symbol.ReturnsByRefReadonly) switch
            {
                (true, true) => (true, true), // but maybe don't come here.
                (true, false) => (true, false),
                (false, true) => (true, true),
